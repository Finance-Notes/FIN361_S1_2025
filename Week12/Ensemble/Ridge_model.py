...
class LassoForecastingEngine:
    ...
    def custom_lasso_validation(self, X_train: np.ndarray, y_train: np.ndarray,
                                ...
                                alpha_grid: np.ndarray) -> Tuple[float, np.ndarray]:
        """
        ...
        for alpha in alpha_grid:
            # Fit Lasso with LARS
            lasso = Ridge(alpha=alpha, max_iter=1000)
        ...
        optimal_alpha = alpha_grid[optimal_idx]

        return optimal_alpha, np.array(validation_scores)
    ...
    def forecast_single_stock(self, features: pd.DataFrame, targets: pd.Series,
        ...
        alpha_grid = np.logspace(-4, 1, 50)  # From 0.0001 to 10
        ...
        for window_end in range(self.min_window_size, n_obs):
            ...
            try:
                optimal_alpha, _ = self.custom_lasso_validation(
                ...
                # Fit final model
                final_model = Ridge(alpha=optimal_alpha, max_iter=1000)
    ...
    def save_detailed_forecasts(self, results: dict, filename: str = 'outputs/detailed_ridge_forecasts.csv') -> pd.DataFrame:
        """
